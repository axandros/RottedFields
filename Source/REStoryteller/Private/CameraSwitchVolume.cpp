...
void ACameraSwitchVolume::OnEndOverlap(AActor * ThisActor, AActor * OtherActor)
{
	...
	// Toggle camera based on players x relative position
	if (PlayerCharacter && OtherActor == PlayerCharacter && PlayerController) {
		...
		if (NewCamera && PlayerRelativeLocation.X > 0) // If the player exits new Camera Side
		{
			PlayerController->SetViewTarget(NewCamera, TransitionParams);
		}
		else if (PreviousCamera && PlayerRelativeLocation.X < 0) // If the player exits new Camera Side
		{
